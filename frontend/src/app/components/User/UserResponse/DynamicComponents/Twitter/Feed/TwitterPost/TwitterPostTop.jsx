...
const TwitterPostTop = ({ id }) => {
  ...
  const FRIENDS =
    socialMediaTranslations?.friends || TW_TRANSLATIONS_DEFAULT.FRIENDS;
  const AND = socialMediaTranslations?.and || TW_TRANSLATIONS_DEFAULT.AND;
  const OTHERS =
    socialMediaTranslations?.others || TW_TRANSLATIONS_DEFAULT.OTHERS;
  const LIKED = socialMediaTranslations?.liked || TW_TRANSLATIONS_DEFAULT.LIKED;
  const RETWEETED =
    socialMediaTranslations?.retweeted || TW_TRANSLATIONS_DEFAULT.RETWEETED;
  const REPORT =
    socialMediaTranslations?.report || TW_TRANSLATIONS_DEFAULT.REPORT;
  const REPORTED =
    socialMediaTranslations?.reported || TW_TRANSLATIONS_DEFAULT.REPORTED;
  ...
  function formLikedOrRetweetStr(by, ships, overflow, likeOrRetweet) {
    ...
    if (by.length === 0 && overflow)
      return `${overflow} ${FRIENDS} ${likeOrRetweet}`;
    const valuesAvail = [];
    ...
    if (valuesAvail.length < 1) {
      ...
      if (overflow) {
        const nameBreak =
          valuesAvail[0].length > 39
            ? valuesAvail[0].substr(0, 36) + "..."
            : valuesAvail[0];
        return `${nameBreak} ${AND} ${overflow} ${OTHERS} ${likeOrRetweet}`;
      } else {
        const nameBreak =
          valuesAvail[0].length > 55
            ? valuesAvail[0].substr(0, 52) + "..."
            : valuesAvail[0];
      ...
      if (overflow) {
        ...
        const nameBreak =
          valuesAvail[0].length > 25
            ? valuesAvail[0].substr(0, 22) + "..."
            : valuesAvail[0];
        const nameBreak2 =
          valuesAvail[1].length > 24
            ? valuesAvail[1].substr(0, 21) + "..."
            : valuesAvail[1];
  ...
  function getLikedOrRetweetedBy() {
    if (
      !userRegisterData.RELATIONSHIP ||
      userRegisterData.RELATIONSHIP.length < 2
    )
      return null;
    const likedBy = singlePost.likedBy ? singlePost.likedBy.split(",") : [];
    const likedByOverflow = parseNumber(singlePost.likedByOverflow)
      ? parseNumber(singlePost.likedByOverflow)
      : null;
    const retweetedBy = singlePost.retweetedBy
      ? singlePost.retweetedBy.split(",")
      : [];
    const retweetedByOverflow = parseNumber(singlePost.retweetedByOverflow)
      ? parseNumber(singlePost.retweetedByOverflow)
      : null;

    if (likedBy.length > 0 || likedByOverflow) {
      setRetweetOrLikeIcon("LIKE");
      return formLikedOrRetweetStr(
        likedBy,
        userRegisterData.RELATIONSHIP,
        likedByOverflow,
        LIKED,
        ...
        <>
          {singlePost.type === "RETWEET" && (
            <div className="twitterRetweeted">
              <div className="twitterRetweetedIcon">
                <InteractionRetweet className="twUserPostRetweet" />
              </div>
              <div className="twitterRetweetedText">
                {socialMediaTranslations?.you_retweeted ||
                  TW_TRANSLATIONS_DEFAULT.YOU_RETWEETED}
              </div>
            </div>
          )}
          ...
          {likedOrRetweetComp ? (
            <div className="twitterRetweeted">
              ...
              <div className="twitterRetweetedText">{likedOrRetweetComp}</div>
            </div>
          ) : null}
              ...
              <div className="twitterPostAvatar">
                {singlePost.attachedAuthorPicture ? (
                  <DynamicMediaProfile
                    ...
                    customCSS="twAuthorProfileImage"
                  />
                ) : (
                  <Avatar
                    src={
              ...
              <div className="twitterPostBody">
                    ...
                    >
                      <MenuItem disabled={singlePost.userPost}>
                        ...
                        >
                          ...
                          />
                          <p className="defaultText report-text-tw">
                            {reportText}
                          </p>
                        </div>
                      </MenuItem>
                ...
                {singlePost.postMessage && (
                  <div className="twitterPostHeaderDescription">
                    <Text
                      postMessage={singlePost.postMessage}
                      link={singlePost.link}
                      customClassName="twitterPostTopText"
                      charLimit={280}
                    />
                  </div>
                )}
                ...
                <div className="twitterPostMediaBox">
                  {(singlePost.type === "PHOTO" ||
                    ...
                    <DynamicMedia
                      ...
                      customCSS="twitterAllRoundBorder"
                    />
                  )}

                  {singlePost.type === "LINK" ? <DynamicLink id={id} /> : null}
