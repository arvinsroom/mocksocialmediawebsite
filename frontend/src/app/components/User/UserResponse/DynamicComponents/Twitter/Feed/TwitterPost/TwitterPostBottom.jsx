        ...
        <div className="twitterPostFooter">
          ...
          >
            <InteractionRetweet
              ...
              className={
                repeatStyle
                  ? "twRetweetHover tweetBoxAllIcon"
                  : "tweetBoxAllIcon"
              }
            />
            &nbsp;
            <span
              className={
                repeatStyle
                  ? "twGreen twitterBottomTextProps"
                  : "twitterBottomTextProps"
              }
            >
          ...
          >
            <InteractionLike
              stroke={
                postMetadata.actionId || favIconStyle ? "#f91880" : "#536471"
              }
              strokeWidth="1.5"
              fill={postMetadata.actionId ? "#f91880" : "none"}
              className={
                favIconStyle ? "twLikeHover tweetBoxAllIcon" : "tweetBoxAllIcon"
              }
            />
            &nbsp;
            <span
              className={
                postMetadata.actionId || favIconStyle
                  ? "twRed twitterBottomTextProps"
                  : "twitterBottomTextProps"
              }
            >
              {postMetadata.actionId
                ? (postMetadata.initLike + 1).toString()
                : postMetadata.initLike.toString()}
            </span>
          </button>

          <button component="label" className="tweetBoxIcons">
            <InteractionShare
              strokeWidth="1.5"
              fill="none"
        ...
        >
          {postMetadata.type === "RETWEET" ? (
            <MenuItem onClick={(e) => handleUndoRetweet(e)}>
              <RepeatOutlinedIcon />{" "}
              {socialMediaTranslations?.undo_retweet ||
                TW_TRANSLATIONS_DEFAULT.UNDO_RETWEET}
            </MenuItem>
          ) : (
            <MenuItem onClick={(e) => handleRetweet(e)}>
              <RepeatOutlinedIcon />{" "}
              {socialMediaTranslations?.retweet ||
                TW_TRANSLATIONS_DEFAULT.RETWEET}
            </MenuItem>
          )}
          <MenuItem onClick={(e) => openModal(e, "QUOTETWEET")}>
            <CreateOutlinedIcon />{" "}
            {socialMediaTranslations?.quote_tweet ||
              TW_TRANSLATIONS_DEFAULT.QUOTE_TWEET}
          </MenuItem>
        </Menu>
        {modalOpen && (
          <Modal
                ...
                <div className="twModalContainerPaper">
                  ...
                  {modalType === "REPLY" ? (
                    <div key={modalType}>
                      <TweetBox
                        placeholderText={
                          socialMediaTranslations?.tweet_your_reply ||
                          TW_TRANSLATIONS_DEFAULT.TWEET_YOUR_REPLY
                        }
                        replyTo={id}
                        quoteTweet={null}
                        retweetParentId={
                          postMetadata.type === "RETWEET"
                            ? postMetadata.parentPostId
                            : null
                        }
                    ...
                    <div key={modalType}>
                      <TweetBox
                        placeholderText={
                          socialMediaTranslations?.add_a_comment ||
                          TW_TRANSLATIONS_DEFAULT.ADD_A_COMMENT
                        }
                        replyTo={null}
                        quoteTweet={id}
                        retweetParentId={
