...
import { getTwitterPosts } from "../../../../../../actions/socialMedia";
    ...
    for (let post in storedPosts) {
      ...
      if (storedPosts[post].isReplyTo === null) {
        ...
        for (let item in storedPosts) {
          if (
            storedPosts[item].isReplyTo !== null &&
            storedPosts[item].parentPostId === storedPosts[post]._id &&
          ...
          postToRender.map((post, index) => {
            ...
            const replies = post.replies.map((reply, replyIndex) => (
              <div
                key={reply._id}
                ref={
                  post?.replies?.length === replyIndex + 1 ? lastPostRef : null
                }
            ...
            return (
              <div
                key={post._id}
                ref={postToRender?.length === index + 1 ? lastPostRef : null}
                className="twitterPostCard hoverEffect"
              >
