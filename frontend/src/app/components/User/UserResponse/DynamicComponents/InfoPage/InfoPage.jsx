...
const InfoPage = ({ data }) => {
  ...
  const fetchInfoDetails = async () => {
    try {
      ...
      if (obj.isFinish) {
        ...
        var utcDateTimeString = utcDateTime
          .toISOString()
          .replace("Z", "")
          .replace("T", " ");
      ...
      if (obj?.socialMediaPageId !== null) {
        const { data: response } = await getFacebookFakeActionPosts(
          obj.socialMediaPageId,
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!requestConsentRes) dispatch(updateFlowActiveState());
        ...
        dispatch(
          updateUserMain({ consent: consentRes === "YES" ? true : false }),
        );
        dispatch(
          showSuccessSnackbar(
  ...
  const capitalize = (str) => {
    ...
    let translate =
      translations?.[str.toLowerCase()] || USER_TRANSLATIONS_DEFAULT?.[str];
  ...
  return (
    <>
      {data?.richText && (
        <>
          <RenderRichTextArea richText={data.richText} /> <br />{" "}
        </>
      )}
        ...
        <div>
          <FormControl component="fieldset">
            <FormLabel component="legend">
              {translations?.[
                "please_indicate_whether_you_consent_to_participate_to_continue."
              ] || USER_TRANSLATIONS_DEFAULT.PROVIDE_CONSENT}
            </FormLabel>
          </FormControl>
          <RadioGroup
          ...
          >
            <FormControlLabel
              ...
              label={
                translations?.i_consent || USER_TRANSLATIONS_DEFAULT.I_CONSENT
              }
            />
            <FormControlLabel
              value={"NO"}
              control={<Radio color="primary" />}
              label={
                translations?.i_do_not_consent ||
                USER_TRANSLATIONS_DEFAULT.I_DO_NOT_CONSENT
              }
            />
          </RadioGroup>
          <br />
        ...
        <>
          {fakeShareSocialMediaPosts?.length > 0 ? (
            <>
              <p>
                <b>
                  {translations?.["shared_posts"] ||
                    USER_TRANSLATIONS_DEFAULT.SHARED_POSTS}
                </b>
              </p>
              {fakeShareSocialMediaPosts.map((item) => (
                <div key={item._id}>
                  <div className="fakePostDetails">
                    <p>
                      {item?.parentUserPost?.linkTitle ||
                        item?.parentUserPost?.postMessage ||
                        ""}
                    </p>
                  </div>
                </div>
            ...
            <p>
              <b>
                {translations?.["no_shared_posts!"] ||
                  USER_TRANSLATIONS_DEFAULT.NO_SHARED_POSTS}
              </b>
            </p>
          )}
          <br />
          {fakeActionSocialMediaPosts?.length > 0 ? (
            <>
              <p>
                <b>
                  {translations?.["liked_or_commented_posts"] ||
                    USER_TRANSLATIONS_DEFAULT.LIKED_OR_COMMENTED_POSTS}
                </b>
              </p>
                ...
                <div key={item._id}>
                  <div className="fakePostDetails">
                    <p>
                      <b>
                        {translations?.action ||
                          USER_TRANSLATIONS_DEFAULT.ACTION}
                        :
                      </b>{" "}
                      ...
                      <p>
                        <b>
                          {translations?.comment ||
                            USER_TRANSLATIONS_DEFAULT.COMMENT}
                          :
                        </b>{" "}
                        {item.comment || ""}
                      </p>
                    ...
                    <p>
                      {item.userPosts?.linkTitle ||
                        item.userPosts?.postMessage ||
                        ""}
                    </p>
                  </div>
                </div>
            ...
            <p>
              <b>
                {translations?.["no_liked_or_commented_posts!"] ||
                  USER_TRANSLATIONS_DEFAULT.NO_LIKED_OR_COMMENTED_POSTS}
              </b>
            </p>
          )}
        </>
      ...
      {responseCode && (
        <div className="showResponseCode">{responseCode || ""}</div>
      )}
