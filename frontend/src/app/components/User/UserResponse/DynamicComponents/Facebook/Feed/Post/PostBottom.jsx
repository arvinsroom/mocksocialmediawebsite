...
const PostBottom = ({ id, omitInteractionBar }) => {
  ...
  const openShareModal = (e) => {
    e.preventDefault();
    if (
      postMetadata.warningLabel === "FOOTNOTE" ||
  ...
  const handleReactions = async (reaction) => {
    ...
    if (postMetadata.actionId)
      await dispatch(unlikeFbPost(postMetadata.actionId, id));
    const data = {
    ...
    <>
        ...
        <div onMouseLeave={() => setDisplay(false)}>
          <div className="postActionsContainer">
            <div className="postAction reactionContainer childrenReactions">
              {display && (
                <FacebookSelector onSelect={handleReactions} iconSize={30} />
              )}
            </div>
            <div className="postAction totalComments">
              <p>
                {(postMetadata?.comments?.length.toString() || "0") +
                  " " +
                  (socialMediaTranslations?.comments ||
                    FB_TRANSLATIONS_DEFAULT?.COMMENTS)}
              </p>
            </div>
          </div>
          <div className="postOptions">
            <div
              className="postOption parentReactions"
            ...
            >
              ...
              <p className={postMetadata.like.toLowerCase() + "Text"}>
                <strong>
                  {postMetadata.like === "default"
                    ? socialMediaTranslations?.["like"] ||
                      FB_TRANSLATIONS_DEFAULT?.["LIKE"]
                    : socialMediaTranslations?.[
                        postMetadata.like.toLowerCase()
                      ] || FB_TRANSLATIONS_DEFAULT?.[postMetadata.like]}
                </strong>
              </p>
            </div>
            <div className="postOption" onClick={toggleComment}>
              ...
              <p>
                <strong>
                  {socialMediaTranslations?.comment ||
                    FB_TRANSLATIONS_DEFAULT.COMMENT}
                </strong>
              </p>
            </div>
            <div className="postOption" onClick={(e) => openShareModal(e)}>
              ...
              <p>
                <strong>
                  {socialMediaTranslations?.share ||
                    FB_TRANSLATIONS_DEFAULT.SHARE}
                </strong>
              </p>
            </div>
          </div>
          ...
          {modalShareOpen && (
            <ShareAnywayModal
              id={id}
              link={postMetadata.checkersLink}
          ...
          {modalOpen && (
            <ShareModal
              id={postMetadata.parentPostId || id}
      ...
      {(openCommentBox || omitInteractionBar) && (
        <div className="comment">
          {postMetadata.comments?.length > 0
            ? postMetadata.comments.map((commentMetaData, index) => (
                <div
                  key={index}
          ...
          placeholder={socialMediaTranslations?.write_a_comment || FB_TRANSLATIONS_DEFAULT.WRITE_A_COMMENT} /> */}
            <InputEmoji
              ...
              theme={"light"}
              placeholder={
                socialMediaTranslations?.write_a_comment ||
                FB_TRANSLATIONS_DEFAULT.WRITE_A_COMMENT
              }
              className="createCommentInputText"
            />
            <button
            ...
            <button
              ...
              type="submit"
            >
              <IconSendViaIconduckSvg fill="gray" />
            </button>
          </div>
        </div>
      )}
