...
const PostBottomComments = ({ commentMetaData }) => {
  ...
  const singleAuthor = commentMetaData.authorId
    ? useSelector((state) =>
        selectSocialMediaAuthor(state, commentMetaData.authorId),
      )
    : null;
  ...
  return (
    <div className="fbListComments">
      {commentMetaData.attachedAuthorPicture ? (
        <DynamicMediaProfile
          attachedMedia={commentMetaData.attachedAuthorPicture}
          customCSS="fbAuthorProfileImage"
        />
      ) : (
        <Avatar
          src={
            commentMetaData.userComment
