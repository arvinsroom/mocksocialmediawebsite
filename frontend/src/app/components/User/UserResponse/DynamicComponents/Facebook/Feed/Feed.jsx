...
import { getFacebookWithCommentsPosts } from "../../../../../../actions/socialMedia";
      ...
      <div className={classes.feed}>
        {allIds.map((postId, index) => {
          if (allIds.length === index + 1) {
            return (
              <div key={postId} ref={lastPostRef} className={classes.post}>
                <PostTop id={postId} />
                <PostBottom
                  id={postId}
                  omitInteractionBar={omitInteractionBar}
                />
              </div>
            );
          } else {
