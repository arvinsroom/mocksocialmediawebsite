...
const Template = () => {
  ...
  const fetch = async () => {
    try {
      ...
      dispatch(
        showErrorSnackbar(
          "Some error occured while fetching template and user information. Please Refresh the page!",
        ),
      );
    }
  };
    ...
    if (allMedia?.length > 0) {
      ...
      allMedia.forEach(async function (fileObj) {
        try {
          ...
          const fileName =
            fileObj._id + "." + (mimeTypeExt[mimeTypeExt.length - 1] || "");
          ...
          dispatch(
            showErrorSnackbar(
              "Some error occured! Only partial files have been download!",
            ),
          );
          zip.generateAsync({ type: "blob" }).then(function (content) {
            saveAs(content, zipFilename);
  ...
  const downloadSpecificTemplate = async (
    templateId,
    templateName,
    totalEntries,
    type,
    ...
    try {
      ...
      const { data: userData } = await fetchTemplateDataAllUser(
        templateId,
        limit,
        offset,
      );
      const { data: userPostsData } = await fetchTemplateDataAllUserPosts(
        templateId,
        limit,
        offset,
      );
      const { data: userPostsActionsData } =
        await fetchTemplateDataAllUserPostsActions(templateId, limit, offset);
      let allUserData = userData?.allUserData || [];
      let allUsersPostsData = userPostsData?.allUsersPostsData || [];
      let allUsersPostsActionsData =
        userPostsActionsData?.allUsersPostsActionsData || [];
      ...
      if (allUserData.length > 0) {
        if (type === "JSON") {
          ...
          while (offset < totalEntries) {
            ...
            const { data: userPostsActionsData } =
              await fetchTemplateDataAllUserPostsActions(
                templateId,
                limit,
                offset,
              );
            allUserData = allUserData.concat(userData?.allUserData || []);
            allUsersPostsData = allUsersPostsData.concat(
              userPostsData?.allUsersPostsData || [],
            );
            allUsersPostsActionsData = allUsersPostsActionsData.concat(
              userPostsActionsData?.allUsersPostsActionsData || [],
            );
            offset = allUserData.length;
          }
          ...
          const { data: socialMediaData } =
            await fetchTemplateDataSocialMedia(templateId);
          const { data: questionData } =
            await fetchTemplateDataQuestionData(templateId);

          const socialMediaPageData =
            socialMediaData?.socialMediaPageData || null;
          const templateAdminPortalQuestionsData =
            questionData?.templateAdminPortalQuestionsData || null;

          // normalize the socialMediaPageIdsArray to make dynamic headers
          const socailMediaDynamicArray =
            formSocialMediaPageIdsArray(socialMediaPageData);

          // normalize the templateAdminPortalQuestionsData to make dynamic headers
          const questionIdsDynamicArray = formQuestionsIdsArray(
            templateAdminPortalQuestionsData,
          );

          const spreadsheetData = [];
          // initially push the dynamic headers array
          const mainHeadersDynamicArray = formulateHeaders(
            questionIdsDynamicArray,
            socailMediaDynamicArray,
          );
          ...
          while (allUserData.length !== 0) {
            ...
            const { userPosts } = allUsersPostsData[i];

            const { userPostActions } = allUsersPostsActionsData[i];
            ...
            if (i === allUserData.length) {
              ...
              const { data: userPostsData } =
                await fetchTemplateDataAllUserPosts(templateId, limit, offset);
              const { data: userPostsActionsData } =
                await fetchTemplateDataAllUserPostsActions(
                  templateId,
                  limit,
    ...
    <Container component="main" maxWidth="lg" className={classes.card}>
      ...
      <div className={classes.form}>
        <Table aria-label="Template(s) with User(s) Information">
          <TableHead>
            <TableRow>
              <TableCell
                ...
                align="center"
              >
                <p>{DATA_PAGE.CONDITION_NAME}</p>
              </TableCell>
              <TableCell
                className={`${classes.body} ${classes.head}`}
                align="center"
              >
                <p>{DATA_PAGE.RESPONSES}</p>
              </TableCell>
              <TableCell
                className={`${classes.body} ${classes.head}`}
                align="center"
          ...
          <TableBody>
            {tempWithUsersCount?.length &&
              tempWithUsersCount.map((row) => (
        ...
        {!tempWithUsersCount && (
          <h5 style={{ textAlign: "center" }}>{DATA_PAGE.NO_RESPONSE_YET}</h5>
        )}
