...
const MCQ = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (OpenTextArr.length > 0) {
        ...
        dispatch(
          showSuccessSnackbar(
            MCQ_PAGE.SUCCESSFULLY_CREATED_MULTIPLE_CHOICE_PAGE,
          ),
        );
        resetValues();
      } else {
  ...
  const handleCustomField = async (customField, type, event) => {
    ...
    if (type === "required" || type === "multiResponse")
      customField[type] = event.target.checked;
    ...
    <Container component="main" maxWidth="lg" className={classes.card}>
      ...
      <form onSubmit={handleSubmit} className={classes.form}>
        ...
        <Button
          ...
          component="label"
          onClick={() => addQuestion()}
          className={classes.marginTenPx}
          startIcon={<IconCirclePlus />}
        >
          CREATE NEW QUESTION
        </Button>
        {OpenTextArr?.length > 0
          ? OpenTextArr.map((question, questionIndex) => (
              <div key={questionIndex} className={classes.card}>
                <Grid container spacing={1}>
                  <Grid item xs={8}>
                    <TextField
                      ...
                      onChange={(e) =>
                        handleCustomField(question, "questionText", e)
                      }
                      autoFocus
                    />
                  </Grid>
                  ...
                  <Grid item xs={1} className={classes.flexCenter}>
                    <Tooltip title="Response required?">
                      <Switch
                        ...
                        onChange={(e) =>
                          handleCustomField(question, "required", e)
                        }
                        color="primary"
                        name="requiredField"
                  ...
                  <Grid item xs={1} className={classes.flexCenter}>
                    <Tooltip title="Allow multiple responses?">
                      <Switch
                        ...
                        onChange={(e) =>
                          handleCustomField(question, "multiResponse", e)
                        }
                        color="primary"
                  ...
                  <Grid
                    item
                    xs={1}
                    className={`${classes.floatRight}, ${classes.flexCenter}`}
                  >
                ...
                OpenTextArr[questionIndex].mcqOptions.length > 0
                  ? OpenTextArr[questionIndex].mcqOptions.map(
                      (option, mcqOptionIndex) => (
                        <Grid key={mcqOptionIndex} container spacing={1}>
                          <Grid item xs={10}>
                            <TextField
                              ...
                              onChange={(e) =>
                                handleCustomField(option, "optionText", e)
                              }
                              autoFocus
                            />
                          </Grid>
                          <Grid item xs={1}>
                            <TextField
                              id="standard-number"
                              onChange={(e) =>
                                handleCustomField(option, "optionOrder", e)
                              }
                              inputProps={{ min: 0, max: 65535, step: 1 }}
                              type="number"
                              label="Order"
                          ...
                          >
                            <Tooltip title="Delete Mcq Option">
                              <Button
                                ...
                                className={classes.floatRight}
                                onClick={() =>
                                  removeMcqOption(questionIndex, mcqOptionIndex)
                                }
                              >
                                <IconTrash />
                              </Button>
