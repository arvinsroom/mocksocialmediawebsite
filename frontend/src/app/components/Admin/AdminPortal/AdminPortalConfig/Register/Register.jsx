...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (customFieldArr.length > 0) {
        ...
        dispatch(
          showSuccessSnackbar(
            REGISTER_PAGE.SUCCESSFULLY_CREATED_REGISTRATION_PAGE,
          ),
        );
        resetValues();
      } else {
  ...
  const handleCustomField = async (customField, type, event) => {
    ...
    if (type === "required" || type === "response")
    ...
    <>
      <Container component="main" maxWidth="lg" className={classes.card}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {customFieldArr?.length > 0
            ? customFieldArr.map((customField, customFieldIndex) => (
                <Container
                ...
                >
                  <Grid container spacing={1}>
                    <Grid item xs={8}>
                      <TextField
                        ...
                        label={"Display Name"}
                        onChange={(e) =>
                          handleCustomField(customField, "displayName", e)
                        }
                        autoFocus
                      />
                    </Grid>
                    <Grid item xs={1}>
                      <TextField
                        id="standard-number"
                        onChange={(e) =>
                          handleCustomField(customField, "order", e)
                        }
                    ...
                    <Grid item xs={1} className={classes.flexCenter}>
                      <Tooltip title="Field required?">
                        <Switch
                          ...
                          onChange={(e) =>
                            handleCustomField(customField, "required", e)
                          }
                          color="primary"
                          name="requiredField"
                    ...
                    <Grid item xs={1} className={classes.flexCenter}>
                      <Tooltip title="Store response to database?">
                        <Switch
                          ...
                          onChange={(e) =>
                            handleCustomField(customField, "response", e)
                          }
                          color="primary"
                          name="responseField"
                          inputProps={{
                            "aria-label": "Store response to database",
                          }}
                        />
                      </Tooltip>
                    </Grid>
                  ...
                  >
                    ...
                    <Select
                      ...
                      onChange={(e) =>
                        handleCustomField(customField, "type", e)
                      }
                      label={"Field Type"}
                    >
                      {customFields}
                  ...
                  >
                    ...
                    <Select
                      ...
                      onChange={(e) =>
                        handleCustomField(customField, "referenceName", e)
                      }
