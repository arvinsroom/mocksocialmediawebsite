...
const Template = () => {
  ...
  const handleTemplateCode = async (templateId, e) => {
    ...
    if (
      Number(changedTemplateCode) &&
      changedTemplateCode >= 100000 &&
      changedTemplateCode <= 999999
    ) {
      const tempObj = {
        _id: templateId,
      ...
      await dispatch(
        updateTemplate({ tempObj }, "Access code successfully updated."),
      );
      setTemplateCodes({});
      await retrivePrevTemplates();
    } else {
      dispatch(
        showInfoSnackbar(
          "Please enter valid Access Code i.e. [100000, 999999]",
        ),
      );
    }
  };
    ...
    <>
      ...
      <Container component="main" maxWidth="lg" className={classes.card}>
        <Table aria-label="Template(s)">
          <TableHead>
            <TableRow>
              ...
              <TableCell
                ...
                align="center"
              >
                <p>{TEMPLATE.SET_AS_ACTIVE}</p>
              </TableCell>
              <TableCell
                className={`${classes.body}, ${classes.head}`}
                align="center"
              >
                <p>{TEMPLATE.ACCESS_CODE}</p>
              </TableCell>
              <TableCell
                className={`${classes.body}, ${classes.head}`}
                align="center"
              >
                <p>{TEMPLATE.CHANGE_ACCESS_CODE}</p>
              </TableCell>
              <TableCell
                className={`${classes.body}, ${classes.head}`}
                align="center"
          ...
          <TableBody>
            {prevTemplates && prevTemplates.length > 0
              ? prevTemplates.map((row) => (
                      ...
                      <Button
                        ...
                        onClick={() =>
                          handleTemplateId(
                            row._id,
                            row.name,
                            TEMPLATE.TEMPLATE_CURRENT_SELECT,
                          )
                        }
                      >
                        <IconEdit />
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            Deleting this condition will also delete all the responses attached
            to this condition. You can check all the responses on the Data tab
            and make sure to download the data before hand. This action cannot
            be undone.
            <br></br>
            <br></br>
